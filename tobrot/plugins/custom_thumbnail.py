...
async def save_thumb_nail(client, message):
    ...
    if message.reply_to_message is not None:
        ...
        await ismgs.edit(
            "<b>Congratulations!</b>\n<code>Custom video / file thumbnail saved!</code>\n"
            + "<i>This image will be used in the upload, till /clearthumbnail.</i>"
        )
    else:
        await ismgs.edit("<b>Reply with a image to save custom thumbnail</b>")
...
async def clear_thumb_nail(client, message):
    ...
    if os.path.exists(thumb_image_path):
        ...
        await ismgs.edit("<i>Custom thumbnail cleared successfully!</i>")
    else:
        await ismgs.edit("<b>Nothing to clear!\nAre u mistake?</b>")
